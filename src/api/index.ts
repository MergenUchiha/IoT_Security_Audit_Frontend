...
export const devicesApi = {
    ...
    create: (dto: CreateDeviceDto) =>
        http.post<Device>("/devices", dto).then((r) => r.data),
    update: (id: string, dto: UpdateDeviceDto) =>
        http.patch<Device>(`/devices/${id}`, dto).then((r) => r.data),
...
export const logsApi = {
    list: (
        deviceId: string,
        params?: { limit?: number; level?: string; from?: string; to?: string },
    ) =>
        http
            .get<LogEntry[]>(`/devices/${deviceId}/logs`, { params })
            .then((r) => r.data),
    ...
    run: (deviceId: string, dto: RunAuditDto) =>
        http
            .post<{
                auditRun: AuditRun;
                summary: unknown;
            }>(`/devices/${deviceId}/audits/run`, dto)
            .then((r) => r.data),
...
export const alertsApi = {
    list: (
        ...
        http
            .get<Alert[]>(`/devices/${deviceId}/alerts`, { params })
            .then((r) => r.data),
    ack: (alertId: string, note?: string) =>
        http
            .patch<Alert>(`/alerts/${alertId}/ack`, { note })
            .then((r) => r.data),
};
...
export const rulesApi = {
    ...
    get: (id: string) =>
        http.get<CorrelationRule>(`/rules/${id}`).then((r) => r.data),
    create: (dto: CreateRuleDto) =>
        http.post<CorrelationRule>("/rules", dto).then((r) => r.data),
...
export const summaryApi = {
    get: (deviceId: string) =>
        http
            .get<DeviceSummary>(`/devices/${deviceId}/summary`)
            .then((r) => r.data),
};
