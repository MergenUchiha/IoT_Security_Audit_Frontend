...
function DeviceForm({
    ...
    const handleSave = () => {
        onSave({
            name: form.name,
            // Пустые строки → undefined, чтобы не нарушать валидацию бэкенда
            ...
            <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5">
                    <Input
                        ...
                        placeholder="router.local (необязательно)"
                    />
                </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
                ...
                <Select
                    ...
                    onChange={(e) => set("logSourceType", e.target.value)}
                >
                    {LOG_SOURCES.map((ls) => (
...
export default function DevicesPage() {
    ...
    const handleUpdate = async (
        dto: CreateDeviceDto & { isActive?: boolean },
    ) => {
        if (!editing) return;
        setSaving(true);
        try {
                                ...
                                >
                                    <Td>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className="font-medium t-text-primary">
                                                {d.name}
                                            </span>
                                        </div>
                                    </Td>
                                    ...
                                    <Td>
                                        <span
                                            className={`text-xs font-mono px-2 py-0.5 rounded-md ${
                                                d.logSourceType === "MQTT"
                                                    ? "bg-purple-500/20 text-purple-300"
                                                    : d.logSourceType === "HTTP"
                                    ...
                                    <Td>
                                        <span
                                            className={`flex items-center gap-1.5 text-xs font-mono ${d.isActive ? "text-green-400" : "t-text-muted"}`}
                                        >
                                            <span
                                                className={`w-1.5 h-1.5 rounded-full ${d.isActive ? "bg-green-400 animate-pulse" : "bg-slate-600"}`}
                                            />
                                            {d.isActive
                                                ? t("active")
                                                : t("inactive")}
