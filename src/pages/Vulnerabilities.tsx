...
import { Shield, HardDrive, Activity, ChevronDown, FileDown, Eye } from 'lucide-react';
...
const Vulnerabilities = () => {
  ...
  const handleViewDetails = (vuln: Vulnerability) => {
    console.log('Viewing details for:', vuln.id);
    alert(`Viewing details for ${vuln.id}\n\n${vuln.description}\n\nImpact: ${vuln.impact}\n\nSolution: ${vuln.solution}`);
  };

  const handleExportCVE = (vuln: Vulnerability) => {
    console.log('Exporting CVE:', vuln.id);
    ...
    const cveData = {
      ...
      devices: vuln.deviceVulns?.map(dv => dv.device?.name).filter(Boolean) || [],
    };

    const dataStr = JSON.stringify(cveData, null, 2);
    const dataUri = 'data:application/json;charset=utf-8,'+ encodeURIComponent(dataStr);
    
    const exportFileDefaultName = `${vuln.id}_export.json`;
                  ...
                  <div className="mt-4 pt-4 border-t border-secondary space-y-4">
                    ...
                    <div className="flex gap-2 mt-4">
                      ...
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          handleExportCVE(vuln);
                        }}
                        className="px-4 py-2 bg-tertiary hover:bg-secondary text-primary rounded font-mono text-sm transition-all border border-primary flex items-center gap-2"
                      >
